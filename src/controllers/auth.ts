...
export const unifiedSignup = catchAsync(async (req: Request, res: Response) => {
  ...
  if (!name || !password) {
    return res
      ...
      .json({
        status: "fail",
        message: "Name and password are required credentials.",
      });
  }
...
export const unifiedLogin = catchAsync(async (req: Request, res: Response) => {
  ...
  if (!identifier || !password) {
    return res
      ...
      .json({
        status: "fail",
        message: "Please provide your account credentials.",
      });
  }
  ...
  async (req: Request, res: Response) => {
    ...
    if (!provider || !providerId || !email) {
      return res
        ...
        .json({
          status: "fail",
          message: "Missing vital provider mapping identifiers.",
        });
    }
    ...
    if (!validProviders.includes(provider)) {
      return res
        ...
        .json({
          status: "fail",
          message: "Invalid OAuth identifier source channel context.",
        });
    }
    ...
    let user = await prisma.user.findFirst({
      where: {
        OR: [specificQueryConstraint, { email }],
      },
    });
    ...
    if (!user) {
      ...
      console.log(
        `🌱 [OAuth Identity Generated] Registered federated profile under target: ${user.email}`,
      );
      ...
      console.log(
        `🔗 [OAuth Federated Link] Linked missing ${provider} provider index to user profile ID: ${user.id}`,
      );
    }
...
/**
 * Recovery Token Ticket Generator (Forgot Password)
 */
  ...
  async (req: Request, res: Response) => {
    ...
    if (!identifier) {
      return res
        ...
        .json({
          status: "fail",
          message: "Please provide your account Email or Phone number.",
        });
    }
    ...
    if (!user) {
      return res
        ...
        .json({
          status: "fail",
          message: "No account found matching that identifier.",
        });
    }
    ...
    if (!user.passwordHash) {
      return res.status(400).json({
        status: "fail",
        message:
          "This account was registered using a third-party provider (OAuth). Please authenticate via your provider panel.",
      });
    }
    ...
    if (isEmail) {
      console.log(
        `📬 [MOCK EMAIL ENGINE] Sent link to mail system pipeline for ${user.email}:`,
      );
      console.log(
        `👉 POST http://localhost:5000/api/v1/auth/reset-password/${rawResetToken}\n`,
      );
    } else {
      console.log(
        `📱 [MOCK SMS GATEWAY] Dispatched text message route alert to mobile line ${user.phone}:`,
      );
      console.log(
        `👉 Use Code token: [ ${rawResetToken} ] inside your mobile UI wrapper input.\n`,
      );
    }
    ...
    res.status(200).json({
      status: "success",
      message:
        "Security verification token successfully generated and dispatched to terminal logs.",
...
export const resetPassword = catchAsync(async (req: Request, res: Response) => {
  ...
  if (!password) {
    return res
      ...
      .json({
        status: "fail",
        message: "Please provide your new password string replacement.",
      });
  }
  ...
  if (!cachedTicket) {
    return res
      ...
      .json({
        status: "fail",
        message: "Token verification failed. Token is invalid or has expired.",
      });
  }
  ...
  if (new Date() > cachedTicket.expiresAt) {
    ...
    return res
      ...
      .json({
        status: "fail",
        message:
          "This recovery session has expired. Please request a new token.",
      });
  }
  ...
  console.log(
    `🧹 [Cache Invalidation] Successfully consumed and purged reset token from volatile memory structures.`,
  );

  res.status(200).json({
    status: "success",
    message:
      "Your security credentials have been mutated successfully. Access grid synchronized.",
  });
});
