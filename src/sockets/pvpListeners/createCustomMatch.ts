...
import type { BattleMode, CustomLobbyState, PlayerProfile } from "../types.js";
...
export default (io: Server, socket: Socket, activeMatches: Map<any, any>) => {
  ...
  socket.on("pvp:startCustom", async (payload: { roomCode: string }) => {
    try {
      ...
      await matchReady(io, socket, activeMatches, roomCode, lobby.players, lobby.subject, lobby.mode, payload.presetConfig);
    } catch (error) {
